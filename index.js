...
function createBankomat(bankNotesRepository, bank) {
    ...
    const bankomat = {
        ...
        giveMoney(count) {
            ...
            const cash = count;
            let rep = Object.entries(this.notesRepository).sort((a, b) => b[0] - a[0]);
            ...
            for (let i = String(count).length; i > 1; i--) {
                ...
                rep.forEach(([banknote, banknoteCount], i) => {
                    if (banknoteCount !== 0 && e >= banknote && banknoteCount * banknote >= e) {
                        if (result[banknote]) {
                            result[banknote] += Math.floor(e / banknote);
                            rep[i] = [banknote, banknoteCount - Math.floor(e / banknote)];
                            sum += banknote * Math.floor(e / banknote);
                            e = e - banknote * Math.floor(e / banknote);
                        } else {
                            result[banknote] = Math.floor(e / banknote);
                            rep[i] = [banknote, banknoteCount - Math.floor(e / banknote)];
                            sum += banknote * Math.floor(e / banknote);
                        ...
                        e = e - banknote * banknoteCount;
                        if (result[banknote]) {
                            result[banknote] += banknoteCount;
                            rep[i] = [banknote, 0];
                        } else {
                            result[banknote] = banknoteCount;
                            rep[i] = [banknote, 0];
...
console.log(crtBank);
console.log(crtBank.giveMoney(33330)); //1 150 000
// console.log(crtBank);
// console.log(crtBank);
